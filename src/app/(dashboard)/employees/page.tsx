...
export default function EmployeesPage() {
  ...
  if (status === "loading") return <div style={{ padding: 40, textAlign: "center", color: "#9CA3AF" }}>Loading…</div>;
  if (!isSuperAdmin) return null;

  const roleLabel: Record<string, string> = { SUPER_ADMIN: "Super Admin", ADMIN: "Admin", EMPLOYEE: "Employee" };
  const roleColors: Record<string, { bg: string; color: string }> = {
    SUPER_ADMIN: { bg: "#EEF1FE", color: "#3451D1" },
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
        ...
        <Link href="/employees/new" style={{ background: "#4F6EF7", color: "#fff", borderRadius: 10, padding: "9px 18px", fontSize: 13, fontWeight: 600, textDecoration: "none" }}>+ Add Employee</Link>
      </div>
              ...
              {employees.map(emp => {
                ...
                return (
                  <tr key={emp.id}
                    ...
                    <td style={{ padding: "12px 16px", fontSize: 13, fontWeight: 600, color: "#374151" }}>{emp.salary ? "₹" + Number(emp.salary).toLocaleString("en-IN") : "—"}</td>
                  </tr>
                );
              })}
            </tbody>
