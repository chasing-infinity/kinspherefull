...
function OnLeaveSection({ data }: any) {
  if (!data.onLeaveNow?.length) return null;
  return (
    <div style={{ background: "#fff", border: "1px solid #F3F4F6", borderRadius: 14, padding: 20, marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
      <h3 style={{ fontSize: 13, fontWeight: 600, color: "#374151", margin: "0 0 14px" }}>🌴 On Leave Today</h3>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
        {data.onLeaveNow.map((l: any) => (
          <div key={l.id} style={{ display: "flex", alignItems: "center", gap: 8, background: "#FFF7ED", border: "1px solid #FED7AA", borderRadius: 10, padding: "8px 14px" }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#F97316", color: "#fff", fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "#9CA3AF" }}>Until {new Date(l.endDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</div>
            </div>
          </div>
        ))}
      </div>
...
function BirthdaysAndAnniversaries({ data }: any) {
  return (
    ...
    <div>
      ...
      {isAdmin && data && (
        <>
          ...
          <BirthdaysAndAnniversaries data={data} />
        </>
      )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 14, marginBottom: 24 }}>
            {(data.leaveBalances || []).map((b: any) => (
              <div key={b.leaveType} style={{ background: "#fff", border: "1px solid #F3F4F6", borderRadius: 14, padding: "18px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                <div style={{ fontSize: 11, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>{b.leaveType === "PAID" ? "EARNED" : b.leaveType}</div>
              ...
              {!data.myRequests?.length ? <p style={{ fontSize: 13, color: "#9CA3AF" }}>No requests yet.</p> :
                data.myRequests.map((r: any) => (
                  <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #F9FAFB" }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 500, color: "#1F2937" }}>{r.leaveType === "PAID" ? "Earned" : r.leaveType.charAt(0) + r.leaveType.slice(1).toLowerCase()} Leave</div>
