...
export default function DashboardPage() {
  ...
  if (loading) return <div style={{ color:"#9CA3AF", paddingTop:40, textAlign:"center" }}>Loading…</div>;

  return (
    <div>
      <div style={{ marginBottom:28 }}>
        <h1 style={{ fontSize:22, fontWeight:700, color:"#111827", margin:0 }}>Good morning, {firstName} 👋</h1>
      ...
      {isAdmin && data && (
        <>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:24 }}>
            <StatCard icon="◎" label="Total Employees"   value={data.stats.totalEmployees}   color="#4F6EF7" />
            <StatCard icon="◷" label="On Leave Today"    value={data.stats.onLeaveToday}     color="#F97316" />
            <StatCard icon="⚑" label="Pending Approvals" value={data.stats.pendingApprovals} color="#F59E0B" />
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:14, padding:20, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
              ...
              {data.recentLeaves.length === 0 ? <p style={{ fontSize:13, color:"#9CA3AF" }}>No pending requests.</p> :
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:14, padding:18, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                ...
                {data.birthdays.length === 0 ? <p style={{ fontSize:13, color:"#9CA3AF" }}>None in the next 30 days.</p> :
                  data.birthdays.map((e: any) => (
                    <div key={e.id} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                      ...
                      <span style={{ fontSize:13, color:"#374151", flex:1 }}>{e.firstName} {e.lastName}</span>
                    </div>
                  ))}
              </div>
              <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:14, padding:18, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                <h3 style={{ fontSize:13, fontWeight:600, color:"#374151", margin:"0 0 12px" }}>🎉 Work Anniversaries</h3>
                {data.anniversaries.length === 0 ? <p style={{ fontSize:13, color:"#9CA3AF" }}>None in the next 30 days.</p> :
        ...
        <>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:14, padding:20 }}>
              ...
              {data.myRequests.length === 0 ? <p style={{ fontSize:13, color:"#9CA3AF" }}>No requests yet.</p> :
            ...
            <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:14, padding:20 }}>
              ...
              {data.upcomingHolidays.length === 0 ? <p style={{ fontSize:13, color:"#9CA3AF" }}>No upcoming holidays.</p> :
