...
function OrgNode({ node, level = 0 }: { node: any; level?: number }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center" }}>
      <div style={{
        background:"#fff",
        border: level === 0 ? `2px solid ${color}` : "1px solid #E5E7EB",
        borderRadius:12,
        padding:"14px 18px",
        minWidth:150,
        textAlign:"center",
        boxShadow: level === 0 ? `0 0 0 4px ${color}15` : "0 1px 4px rgba(0,0,0,0.06)"
      }}>
        <div style={{
          width:40, height:40, borderRadius:"50%",
          background:color+"18", color, fontSize:14, fontWeight:700,
        ...
        {node.designation && (
          <div style={{ fontSize:11, color:"#6B7280", marginTop:2 }}>{node.designation}</div>
        )}
  ...
  return (
    <div>
      <div style={{ marginBottom:28 }}>
        ...
        <p style={{ fontSize:13, color:"#9CA3AF", marginTop:4 }}>Bipolar Factory reporting hierarchy</p>
      </div>
      <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:14, padding:40, overflowX:"auto", boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
        {loading ? (
          ...
          <div style={{ textAlign:"center", color:"#9CA3AF", padding:40 }}>No employees found.</div>
        ) : (
