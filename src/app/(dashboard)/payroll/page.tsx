...
function PayslipModal({ slip, onClose }: any) {
  ...
  useEffect(() => {
    fetch(`/api/payroll/payslips/${slip.id}`).then(r=>r.json()).then(setDetail);
  }, [slip.id]);
  if (!detail) return <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100 }}><div style={{ background:"#fff", borderRadius:16, padding:32, color:"#9CA3AF" }}>Loading…</div></div>;
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100, padding:24 }} onClick={onClose}>
      <div style={{ background:"#fff", borderRadius:18, width:520, maxHeight:"90vh", overflow:"auto", boxShadow:"0 24px 60px rgba(0,0,0,0.2)" }} onClick={e=>e.stopPropagation()}>
        <div style={{ background:"#4F6EF7", borderRadius:"18px 18px 0 0", padding:"24px 28px", color:"#fff" }}>
          ...
          <div style={{ marginTop:12, fontSize:12, opacity:0.7 }}>{detail.employee?.department?.name} · {detail.employee?.user?.email}</div>
        </div>
...
function ConfigModal({ emp, onDone, onClose }: any) {
  const [form, setForm] = useState({ annualCTC: emp.payrollConfig?.annualCTC || "", basicPercent: emp.payrollConfig?.basicPercent || 40, hraPercent: emp.payrollConfig?.hraPercent || 20, pfDeduction: emp.payrollConfig?.pfDeduction || 1800, professionalTax: emp.payrollConfig?.professionalTax || 200, incomeTax: emp.payrollConfig?.incomeTax || 0 });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const monthly = Number(form.annualCTC) / 12;
  const basic = Math.round(monthly * Number(form.basicPercent) / 100);
  const hra = Math.round(monthly * Number(form.hraPercent) / 100);
  const other = Math.round(monthly - basic - hra);
  const net = Math.round(monthly - Number(form.pfDeduction) - Number(form.professionalTax) - Number(form.incomeTax));
  const valid = Number(form.basicPercent) + Number(form.hraPercent) <= 100 && Number(form.basicPercent) > 0 && Number(form.annualCTC) > 0;

  const save = async () => {
    setSaving(true); setError("");
    const res = await fetch("/api/payroll/config", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ employeeId: emp.id, ...form, annualCTC: Number(form.annualCTC), basicPercent: Number(form.basicPercent), hraPercent: Number(form.hraPercent), pfDeduction: Number(form.pfDeduction), professionalTax: Number(form.professionalTax), incomeTax: Number(form.incomeTax) }) });
    if (res.ok) { onDone(); onClose(); } else { const d = await res.json(); setError(d.error||"Failed to save"); }
    setSaving(false);
  };
      ...
      <div style={{ background:"#fff", borderRadius:18, width:480, boxShadow:"0 20px 50px rgba(0,0,0,0.15)" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ padding:"20px 28px" }}>
          <div style={{ marginBottom:14 }}>
            ...
            <input type="number" value={form.annualCTC} onChange={e=>setForm(f=>({...f,annualCTC:e.target.value}))}
              placeholder="e.g. 1200000" style={{ width:"100%", padding:"9px 12px", border:"1px solid #E5E7EB", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box" }} />
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:10 }}>
            <div>
              <label style={{ fontSize:12, fontWeight:600, color:"#374151", display:"block", marginBottom:5 }}>Basic %</label>
              <input type="number" min="1" max="80" value={form.basicPercent} onChange={e=>setForm(f=>({...f,basicPercent:e.target.value}))} style={{ width:"100%", padding:"9px 12px", border:"1px solid #E5E7EB", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box" }} />
            </div>
            <div>
              <label style={{ fontSize:12, fontWeight:600, color:"#374151", display:"block", marginBottom:5 }}>HRA %</label>
              <input type="number" min="0" max="50" value={form.hraPercent} onChange={e=>setForm(f=>({...f,hraPercent:e.target.value}))} style={{ width:"100%", padding:"9px 12px", border:"1px solid #E5E7EB", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box" }} />
            </div>
          </div>
          {Number(form.annualCTC) > 0 && (
            <div style={{ background:"#F9FAFB", borderRadius:10, padding:"12px 14px", marginBottom:14, fontSize:12 }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:8 }}>
                {[{l:"Basic",v:basic,c:"#4F6EF7"},{l:"HRA",v:hra,c:"#8B5CF6"},{l:"Other",v:other,c:"#F97316"}].map(r=>(
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:14 }}>
            {[["PF","pfDeduction"],["Prof Tax","professionalTax"],["TDS","incomeTax"]].map(([l,k])=>(
              <div key={k}>
                <label style={{ fontSize:11, color:"#6B7280", display:"block", marginBottom:4 }}>{l}</label>
                <input type="number" min="0" value={(form as any)[k]} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))} style={{ width:"100%", padding:"7px 10px", border:"1px solid #E5E7EB", borderRadius:8, fontSize:12, outline:"none", boxSizing:"border-box" }} />
              </div>
            ))}
          </div>
          {Number(form.annualCTC)>0 && <div style={{ background:"#EEF1FE", borderRadius:10, padding:"10px 14px", display:"flex", justifyContent:"space-between" }}>
...
export default function PayrollPage() {
  ...
  const [tab, setTab] = useState(isAdmin ? "payslips" : "payslips");
  ...
  const loadPayslips = () => {
    setLoading(true);
    fetch(`/api/payroll/payslips?year=${year}`).then(r=>r.json()).then(d=>setPayslips(Array.isArray(d)?d:[])).finally(()=>setLoading(false));
  };
  const loadConfig = () => {
    fetch("/api/payroll/config").then(r=>r.json()).then(setConfigData);
  };

  useEffect(() => { loadPayslips(); }, [year]);
  useEffect(() => { if (isAdmin && tab==="config") loadConfig(); }, [tab, isAdmin]);
                ...
                {payslips.length===0 ? (
                  ...
                  <tr key={slip.id} style={{ borderTop:"1px solid #F9FAFB", cursor:"pointer" }}
                    ...
                    <td style={{ padding:"12px 16px" }}>
                      ...
                      <div style={{ fontSize:11, color:"#9CA3AF" }}>Generated 15th</div>
                    </td>
