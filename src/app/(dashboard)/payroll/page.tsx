...
function PayslipModal({ slip, onClose }: any) {
  ...
  useEffect(() => {
    fetch(`/api/payroll/payslips/${slip.id}`).then(r => r.json()).then(setDetail).catch(console.error);
  }, [slip.id]);
...
export default function PayrollPage() {
  ...
  const loadConfig = () => {
    fetch("/api/payroll/config").then(r => r.json()).then(setConfigData).catch(console.error);
  };
  ...
  useEffect(() => {
    if (status === "loading") return;
    if (isSuperAdmin && tab === "config") loadConfig();
  }, [tab, isSuperAdmin, status]);

  if (status === "loading") return <div style={{ color: "#9CA3AF", paddingTop: 40, textAlign: "center" }}>Loading…</div>;
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "#9CA3AF", marginTop: 4 }}>
            {isSuperAdmin ? "Manage salary configs · payslips auto-generate on the 15th" : "Your payslips — generated on the 15th each month"}
          </p>
        </div>
      ...
      {isSuperAdmin && tab === "config" && configData && (
      ...
      {/* Payslips — everyone sees own, Super Admin sees all */}
      {tab === "payslips" && (
        <div style={{ background: "#fff", border: "1px solid #F3F4F6", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
          {loading ? <div style={{ padding: 40, textAlign: "center", color: "#9CA3AF" }}>Loading payslips…</div> : (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr style={{ background: "#F9FAFB" }}>
                {[isSuperAdmin ? "Employee" : "", "Month", "Gross", "Deductions", "Net Pay", ""].filter(Boolean).map(h => (
                  <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.05em" }}>{h}</th>
                ))}
              </tr></thead>
              <tbody>
